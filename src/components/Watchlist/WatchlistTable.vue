...
export default {
  ...
  methods: {
    async deleteItem(ticker) {
      ...
      if (confirmation) {
        const apiUrl = `/api/watch/delete/${this.useremail}/${ticker}`;
    ...
    async fetchData() {
      ...
      try {
        const apiUrl = `/api/watch/read/${this.useremail}`;
        console.log(apiUrl);
    ...
    async getStockPrice() {
      try {
        for (const item of this.watchlistData) {
          const apiUrl = `/api/yfinance/curentPrice/${item.ticker}`;
    ...
    async getMarketCap() {
      try {
        for (const item of this.watchlistData) {
          const apiUrl = `/api/yfinance/marketCapData/${item.ticker}`;
          const response = await axios.get(apiUrl);
    ...
    async getAvgVolume() {
      try {
        for (const item of this.watchlistData) {
          const apiUrl = `/api/yfinance/averageVolumeData/${item.ticker}`;
          const response = await axios.get(apiUrl);
    ...
    async getPercentChange() {
      try {
        for (const item of this.watchlistData) {
          const apiUrl = `/api/yfinance/percentageChange/${item.ticker}`;
          const response = await axios.get(apiUrl);
